...
- [🚀 全平台部署与上线保姆级实操教程](#-全平台部署与上线保姆级实操教程)
  ...
  - [5. 私有 VPS / Nginx / 本地开发部署](#5-私有-vps--nginx--banner-本地开发部署)
...
2. **整站统一圆形按钮与无描边动态毛玻璃体系**：
   - 右下角主题切换（太阳/月亮）与一键平滑置顶按钮统一升级为**纯净无描边圆形毛玻璃按键**（`rounded-full site-glass border-0`），去除生硬边框；
   - **外观设置面板支持实时调节整站毛玻璃样式**：滑块支持调节模糊强度（0px ~ 32px）与表面通透度（10% ~ 95%），通过 CSS 变量 `--glass-blur`、`--glass-opacity`、`--glass-bg-dark`、`--glass-bg-light` 全局无缝联动，并提供「清透灵动」、「经典毛玻璃」、「深度雾感」、「超清无模糊」四套一键预设。
3. **所有模态窗口（Modal System）统一规范**：
...
11. **实用多功能右侧抽屉（Widgets Drawer）**：
    - 内置待办事项（支持“今日待办”、“最近七天”、“全部”分类筛选与动态数量统计）、随笔便签记事本、解压电子木鱼、实时热搜榜单。

---

## 📁 深入全景：项目所有文件功能与作用详解（小白必读）

为了让任何零基础新手、前端学习者与二次开发者能够清晰理解代码架构，下面将项目中**每一个文件与文件夹**的作用、内部运行逻辑与修改指引进行拆解：
...
- **功能细节**：
  ...
  - 采用无边框高定毛玻璃容器（`.site-glass border-0`），移除了生硬的背景色块填充，使图标与背景浑然一体。
...
- **作用**：右侧滑出的轻量小工具集合。
- **功能细节**：
  - 内置：**便签记事本**（自动持久化随笔草稿）、**待办清单 (Todo)**（支持“今日待办”、“最近七天”、“全部”分类筛选与统计）、**解压电子木鱼**（敲击计数与声音回馈）、**实时聚合热搜榜单**。
...
- **无描边圆形毛玻璃按钮规范**：
  ...
  - 搭配 `.site-glass border-0` 无描边毛玻璃、微光投影与 `hover:scale-110 active:scale-95` 灵动触感反馈。
- **CSS 变量全局联动控制**：
  - `--glass-blur` 控制高斯模糊度（例如 `16px`）；
  - `--glass-opacity` 控制背景表层透明度；
  - `.site-glass-dark` 与 `.site-glass-light` 分别配合变量动态生成半透明亚克力涂层，确保无生硬底色。
- **全站弹窗模态规范 (Modal System)**：
  - **遮罩层**：`fixed inset-0 z-50 bg-black/65 backdrop-blur-md`；
  - **窗口尺寸**：统一为 `max-w-2xl max-h-[88vh] rounded-3xl site-glass border-0`，圆角弧度与留白内外均衡；
